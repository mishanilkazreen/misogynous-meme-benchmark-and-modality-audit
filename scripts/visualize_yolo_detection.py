...
def create_row_visualization(  # pylint: disable=too-many-locals
    ...
    vis_orig = add_label(
        img_orig, f"Pred: {res_orig.predicted_class} ({time_orig:.0f}ms)")
    vis_prep = add_label(
        img_prep, f"Pred: {res_prep.predicted_class} ({time_prep:.0f}ms)")
    gt_img = add_label(condition, f"Ground Truth: {actual}")

    # Info bar
    correct_orig = "OK" if res_orig.predicted_class == actual else "WRONG"
    correct_prep = "OK" if res_prep.predicted_class == actual else "WRONG"
    info = f"Sample {sample['index']} | Digit: {actual} | "
    info += f"Vis: {sample['visibility']} | Orig: {correct_orig} | Prep: {correct_prep}"
...
def build_visualization(
    ...
    return np.vstack([title_bar, header] + rows)
...
def main() -> None:  # pylint: disable=too-many-locals
    ...
    print("\nNote: Model has untrained weights (random predictions)")
