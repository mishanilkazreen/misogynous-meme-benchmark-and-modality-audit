...
def scan_dataset_directory(dataset_dir):
    ...
    return data, sorted(list(classes_set))
...
def get_embeddings_with_cache(dataset_dir, backbone_model_name, image_size, device, use_cache=True):
        ...
        try:
            ...
            if len(scanned_files) == len(cached_files_dict):
                ...
                for item in scanned_files:
                    ...
                    if cached_files_dict[path]['mtime'] != item['mtime'] or cached_files_dict[path]['label'] != item['label']:
                        match = False
                        break
                if match:
    ...
    model, emb_dim = get_feature_extractor(backbone_model_name, pretrained=True)
    ...
    if use_cache:
        files_metadata = {
            item['path']: {'mtime': item['mtime'], 'label': item['label']}
            for item in scanned_files
        }
        save_data = {
...
def train_deep_model(model_spec, data_cfg, train_loader, val_loader, num_classes, output_dir, device):
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"  Epoch {epoch}/{epochs} - Loss: {epoch_train_loss:.4f} | Val Loss: {epoch_val_loss:.4f} | Val Acc: {epoch_val_acc:.4f}")
...
def run_classification_benchmark(data_cfg, model_cfg, output_dir, init_config=None):
    ...
    if mode == 'extract_features':
        ...
        X_paths = sorted(list(embeddings.keys()))
        ...
        if is_pre_split:
            ...
            print(f"Dataset splits resolved from folder structure:")
        ...
        # Fit models
        df_results, predictions_df = train_benchmark_models(
            X_train_scaled, y_train, X_test_scaled, y_test, model_cfg, output_dir, feat_labels=feat_labels
        )
        ...
        train_dataset = ImageClassificationDataset(train_files, class_to_idx, transform=train_transform)
