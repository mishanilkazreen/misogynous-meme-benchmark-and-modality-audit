...
from sklearn import linear_model, svm, discriminant_analysis
...
def train_single_model(model_name, model, X_train, y_train, X_test, y_test, model_cfg, is_multiclass, inner_n_jobs=1):
    """
    ...
    if cv_settings.get('run_cv', True):
        try:
            cv_scores = cross_val_score(best_model, X_train, y_train, cv=cv_folds, scoring=scoring_metric, n_jobs=inner_n_jobs)
...
def _train_task(model_name, model_obj, X_train, y_train, X_test, y_test, model_cfg, is_multiclass):
    ...
    if model_name in ['Multinomial Naive Bayes', 'Complement Naive Bayes']:
        ...
        if min_val < 0:
            print(f"  Skipping {model_name}: Features contain negative values (not supported by MultinomialNB/ComplementNB).")
...
def train_benchmark_models(X_train, y_train, X_test, y_test, model_cfg, output_dir, feat_labels=None):
    """
    ...
    parallel_results = Parallel(n_jobs=-1)(
        delayed(_train_task)(
            model_name, model_obj, X_train, y_train_encoded, X_test, y_test_encoded, model_cfg, is_multiclass
        )
    ...
    for model_name, fitted_model, preds, res in parallel_results:
        if fitted_model is not None:
            ...
            if model_cfg.get('save_models', True):
                ...
                model_path = os.path.join(models_subfolder, f"{model_cfg['config_name']}_{safe_name}.joblib")
