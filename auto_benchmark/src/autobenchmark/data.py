...
def prepare_data(df, data_config, init_config=None):
    ...
    if data_type in ['text', 'image', 'multimodal']:
        ...
        if text_col and text_col in df.columns:
            ...
            if text_features_mode in ['tfidf', 'both']:
                ...
                cleaned_train = Parallel(n_jobs=-1)(delayed(clean_text_string)(t) for t in texts_train)
                cleaned_test = Parallel(n_jobs=-1)(delayed(clean_text_string)(t) for t in texts_test)
            ...
            if text_features_mode in ['embeddings', 'both']:
                ...
                cache_file = os.path.join(cache_dir, f"embeddings_{corpus_hash}_{emb_model_safe}.npz")
                ...
                if os.path.exists(cache_file):
                    ...
                    try:
                        ...
                        model = SentenceTransformer(emb_model_name, device=device, local_files_only=True)
        ...
        if image_col and image_col in df.columns:
            ...
            cache_dir = os.path.join(base_dir, 'results', 'cache')
            os.makedirs(cache_dir, exist_ok=True)
            cache_file = os.path.join(cache_dir, f"image_embeddings_{img_hash}_{backbone_safe}_{image_size}.npz")
    ...
    if use_smote and classification_type in ['binary', 'multiclass']:
        ...
        if np.isnan(X_train_prep).any():
            print("Warning: Missing values found in preprocessed data. Imputing temporarily with 0 for SMOTE.")
