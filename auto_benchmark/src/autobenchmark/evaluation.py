...
def save_and_rank_results(df_results, model_cfg, output_dir):
    ...
    best_model_info = {
        ...
        'Accuracy': best_row['Accuracy'] if 'Accuracy' in best_row else None,
        'F1': best_row['F1'] if 'F1' in best_row else None,
        'ROC_AUC': best_row['ROC_AUC'] if 'ROC_AUC' in best_row else None,
        'Training_Time': best_row['Training_Time'] if 'Training_Time' in best_row else None,
        'Hyperparameters': best_row['Hyperparameters'] if 'Hyperparameters' in best_row else None
    }
...
def generate_evaluation_plots(df_results, output_dir, model_cfg):
    ...
    col_map = {'F1': 'F1', 'ACCURACY': 'Accuracy', 'PRECISION': 'Precision', 'RECALL': 'Recall', 'ROC_AUC': 'ROC_AUC'}
    ...
    if not df_results.empty:
        try:
            ...
            if metric_col not in df_results.columns:
                metric_col = 'Accuracy' if 'Accuracy' in df_results.columns else df_results.columns[2]
            
            df_plot = df_results.sort_values(by=metric_col, ascending=True)
            
            plt.figure(figsize=(10, 6))
            colors = plt.cm.viridis(np.linspace(0.2, 0.8, len(df_plot)))
            bars = plt.barh(df_plot['Model'], df_plot[metric_col], color=colors, edgecolor='grey', height=0.6)
