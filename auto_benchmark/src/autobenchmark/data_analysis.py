...
def run_data_profiling(df, data_config, output_dir):
    ...
    if target_col in df.columns:
        ...
        try:
            ...
            target_counts.plot(kind='pie', autopct='%1.1f%%', startangle=90, colors=['skyblue', 'lightcoral', 'lightgreen'])
    ...
    if wc_cfg.get('enabled', False):
        ...
        if text_column and text_column in df.columns:
            ...
            def create_wc(texts, title, filename):
                ...
                if text_block.strip():
                    wc = WordCloud(width=800, height=400, background_color='white', stopwords=custom_stopwords, colormap='viridis').generate(text_block)
            ...
            if target_column in df.columns:
                ...
                for cls in unique_classes:
                    ...
                    create_wc(cls_df[text_column], f"Word Cloud for Label: {cls}", f"wordcloud_class_{cls}.png")
        else:
            print(f"Warning: Text column '{text_column}' not found in dataset columns for word cloud.")
