...
def load_vlm(model_id, model_type, device, quantization=None):
    ...
    if model_type == 'florence':
        ...
        try:
            ...
            def EncoderDecoderCache_getitem(self, index):
                ...
                if self_layer.keys is None:
                    return None
                self_attn = (self_layer.keys, self_layer.values, getattr(self_layer, '_sliding_window_tensor', None))
                cross_attn = (cross_layer.keys, cross_layer.values, getattr(cross_layer, '_sliding_window_tensor', None))
        ...
        try:
            ...
            get_class_from_dynamic_module('modeling_florence2.Florence2ForConditionalGeneration', model_id)
            ...
            if modeling_module is not None:
                ...
                if cls is not None:
                    ...
                    cls._supports_sdpa = property(lambda self: self.language_model._supports_sdpa if hasattr(self, 'language_model') else False)
                    cls._supports_flash_attn_2 = property(lambda self: self.language_model._supports_flash_attn_2 if hasattr(self, 'language_model') else False)
        ...
        try:
            if hasattr(model, 'language_model'):
                ...
                def patched_prepare(self, decoder_input_ids, past_key_values=None, **kwargs):
                    ...
                    model_inputs = original_prepare(self, decoder_input_ids=decoder_input_ids, **kwargs)
                    return model_inputs
...
def run_vlm_benchmark(data_cfg, model_cfg, output_dir, init_config=None):
    ...
    predictions_df = pd.DataFrame({
        'filename': df_ann[image_col],
        'ground_truth': df_ann[label_col]
    })
    ...
    for model_spec in models_list:
        ...
        for idx, row in df_ann.iterrows():
            ...
            try:
                print(f"  [{idx+1}/{len(df_ann)}] {img_name} -> Raw: '{raw_pred}' | Parsed: '{parsed}' | Time: {latency:.2f}s")
            except UnicodeEncodeError:
                safe_pred = raw_pred.encode('ascii', errors='replace').decode('ascii')
                print(f"  [{idx+1}/{len(df_ann)}] {img_name} -> Raw: '{safe_pred}' | Parsed: '{parsed}' | Time: {latency:.2f}s")
        ...
        if not valid_indices:
            print(f"  WARNING: Model '{model_name}' produced no parsable predictions matching target classes.")
            evaluation_results.append({
                'Model': model_name,
                'Model_ID': model_id,
        ...
        acc = accuracy_score(ground_truth, [val if val is not None else "UNPARSED" for val in parsed_labels])
        precision, recall, f1, _ = precision_recall_fscore_support(gt_filtered, pred_filtered, average='macro', zero_division=0)
        avg_latency = np.mean(latencies)
        
        print(f"  {model_name} Results: Accuracy={acc:.4f}, Precision={precision:.4f}, Recall={recall:.4f}, F1={f1:.4f}, Avg Latency={avg_latency:.2f}s")
...
def _generate_vlm_bar_chart(df_eval, metric_name, output_dir, config_name):
    ...
    fig, ax = plt.subplots(figsize=(10, max(4, len(successful) * 0.6)))
    ...
    for bar, val in zip(bars, successful[metric_name]):
