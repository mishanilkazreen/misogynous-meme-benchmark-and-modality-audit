...
class TestAutobenchmark(unittest.TestCase):
    ...
    def test_prepare_data(self):
        X_train, X_test, y_train, y_test, feat_labels, preprocessor = prepare_data(self.df, self.data_config)
    ...
    def test_train_single_model_and_uncertainty(self):
        X_train, X_test, y_train, y_test, feat_labels, _ = prepare_data(self.df, self.data_config)
        model = LogisticRegression(random_state=42)
        
        fitted_model, preds, metrics = train_single_model(
            'Logistic Regression', model, X_train, y_train, X_test, y_test, self.model_config, is_multiclass=False
        )
        ...
        df_unc = calculate_prediction_uncertainty(fitted_model, 'Logistic Regression', X_test, y_test, output_dir, self.model_config)
    ...
    def test_save_and_rank_results(self):
        df_results = pd.DataFrame([
            {'Model': 'ModelA', 'Accuracy': 0.85, 'F1': 0.82, 'Training_Time': 0.1, 'Hyperparameters': 'default'},
            {'Model': 'ModelB', 'Accuracy': 0.90, 'F1': 0.89, 'Training_Time': 0.2, 'Hyperparameters': 'default'},
            {'Model': 'ModelC', 'Accuracy': 0.78, 'F1': 0.75, 'Training_Time': 0.05, 'Hyperparameters': 'default'}
        ])
    ...
    def test_yolo_helpers(self):
        from autobenchmark.yolo_models import _safe_filename, _compute_f1, resolve_data_yaml
